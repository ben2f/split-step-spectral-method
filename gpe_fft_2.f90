...
MODULE COMM_DATA
	...
	integer, parameter			:: INTERACTION=2*100000
...
MODULE GPE_DATA
  ...
  double precision, parameter :: NU = 0.5D0, NU2 = NU * NU
  double precision, parameter :: G0=-5.0D0,G01=0.03D0,GAM_EFF0=0.01D0

  double precision ::x(NX),x2(NX)
  double precision ::kx(NX),kx2(NX)
  double precision ::V(NX),R2(NX)
  double complex   ::CP(NX),KIN(NX),HX(NX)
  double precision::G, G1, GAM_EFF
...
!------------------------------------------------------------------------------------------------------------------------
	...
	implicit none
!----------------------------------------------------------------------------------------------------------------------------	
	integer :: NRUN=25*10000,LSTORE=1000
	integer :: i,L,step_counter=0.0
	double precision :: t,t_max,tsteps,T1,T2
	double precision ::GSTP,GAMSTP,G1STP,ZNORM, MU, EN,RMS
	double complex	 ::S(NX),STMP(NX)
...
!-----------------------------------------------------------------------------------------------------------------------------
	...
	WRITE(7,*) '  Nonlinearity G1 =',G01
	WRITE(7,*) '  Nonlinearity GAM_EFF =',GAM_EFF0
	...
	!t=0.0D0
  	WRITE (7,*)'--------------------------------------------------------------------------------------------------'
	WRITE (7,*)'Norm',"				       ",'MU'," 			      ",' EN',"			       ",'RMS',"			       ",'|Psi(0)|^2'
	WRITE (7,*)'---------------------------------------------------------------------------------------------------'
	WRITE (7,*) ZNORM, MU, EN, RMS, CP2(NX2) 
...
!--------------------------------------------------   
  ...
  DO i = 1, NX
    !TMP1D(i) = V(i) + G * P2(i)
	TMP1D(i) = V(i) + GMCIG1 * P2(i) + CI * GAM_EFF
