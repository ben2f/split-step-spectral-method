...
MODULE COMM_DATA
  INTEGER, PARAMETER :: NRUN = 10*10000, LSTORE = 1000, IMSTORE = NRUN / 5
...
!  			
  ...
  REAL (8), PARAMETER :: G0 = -2.5D0, G01 = 0.0D0, GAM_EFF0 = 0.0D0
...
! Initial Profile
  CP2 = REAL(CP) * REAL(CP) + AIMAG(CP) * AIMAG(CP)
  DO I = 1, NX
    WRITE(50, 2001) X(I),  CP2(I)
  END DO
  WRITE(50, 2001) X(NX) + DX, CP2(1) ! to makeup last point
  CLOSE(50)
! 
...
!
    DO L = 1, NRUN
		...
		IF (MOD(L, LSTORE) == 0) THEN 
		  ...
		  OPEN(16, file='real1d-rms-g=-2.5.txt', status='unknown')
          
		  WRITE(16, 2222) TI,CP2(NX2)
		  WRITE(100, 2222) TI,RMS
